...
class CGASModule1:
    ...
    def _extract_cp_reads(self, sample_name: str, bam_file: Path,
        ...
        # Step 1: Filter to mapped reads only.
        # -F 4    : exclude reads where the read itself is unmapped
        # -F 2048 : exclude supplementary/chimeric alignments
        # Flag 8 (mate unmapped) is intentionally NOT filtered so that
        # half-mapping pairs are retained — the -N flag below handles them.
        mapped_bam = cp_reads_sample_dir / "mapped.bam"
        subprocess.run(
            [self.samtools_path, "view", "-b", "-F", "2052",
             str(bam_file), "-o", str(mapped_bam)],
            check=True, capture_output=True
        )

        # Step 2: Name-sort the filtered BAM before FASTQ conversion.
        # *** THIS IS THE PRIMARY BUG FIX ***
        # The incoming bam_file is coordinate-sorted (samtools sort by position).
        # samtools fastq on a coordinate-sorted BAM cannot reliably pair mates
        # because both mates of a pair are separated by their mapping positions
        # (potentially tens of kb apart in the chloroplast). samtools flushes
        # its internal pairing buffer before seeing the second mate, so the
        # vast majority of pairs are silently dropped — producing only ~488
        # reads from 269,856 mapped (the exact symptom observed).
        # Name-sorting guarantees both mates are adjacent so samtools fastq
        # sees every pair together and outputs all reads correctly.
        namesorted_bam = cp_reads_sample_dir / "mapped.namesorted.bam"
        subprocess.run(
            [self.samtools_path, "sort", "-n",
             "-@", str(self.threads),
             str(mapped_bam), "-o", str(namesorted_bam)],
            check=True, capture_output=True
        )
        mapped_bam.unlink()

        # Step 3: Convert name-sorted BAM to FASTQ.
        cp_r1 = cp_reads_sample_dir / f"{sample_name}_cp_R1.fq.gz"

        if is_paired:
            cp_r2 = cp_reads_sample_dir / f"{sample_name}_cp_R2.fq.gz"
            # -N : write reads whose mate is absent/unmapped to R1/R2 rather than
            #      discarding them to -0 /dev/null (prevents half-mapping pair loss)
            # -0 /dev/null : discard true unpaired/other reads
            # -s /dev/null : discard singletons without a mate record in the BAM
            subprocess.run(
                [self.samtools_path, "fastq",
                 "-N",
                 "-1", str(cp_r1),
                 "-2", str(cp_r2),
                 "-0", "/dev/null",
            ...
            # Single-end: pipe through gzip so the .fq.gz is genuinely compressed.
            # Writing raw samtools stdout directly to a .gz path produces an
            # uncompressed file with a misleading extension that breaks downstream tools.
            fastq_proc = subprocess.Popen(
                [self.samtools_path, "fastq", str(namesorted_bam)],
            ...
            with open(cp_r1, "wb") as out_fq:
                ...
                fastq_proc.stdout.close()
                gzip_proc.communicate()
            fastq_proc.wait()
            if fastq_proc.returncode != 0:
                raise subprocess.CalledProcessError(fastq_proc.returncode, "samtools fastq")
            self.logger.info(f"Extracted single-end CP reads: {cp_r1.name}")

        namesorted_bam.unlink()

...
class CGASModule2:
    ...
    def _process_pga_output(self, pga_output_dir: Path, input_files: List[Path]):
        ...
        for gb_file in gb_files:
            try:
                ...
                self._fix_rps12_trans_splicing(final_genbank_path)
    ...
    def _fix_rps12_trans_splicing(self, genbank_file: Path):
        """
        Fix rps12 trans-spliced gene annotation in GenBank file.

        rps12 is a trans-spliced gene in plastomes:
          - Exon 1 (LSC part): one fragment in the LSC region
          - Exons 2 & 3 (IR parts): two fragments in IRb AND a mirror copy in IRa

        PGA annotates rps12 as separate, independent genes (one per region).
        This function removes those separate entries and replaces them with two
        properly joined, trans-spliced records:
          1. join(LSC_exon1, IRb_exon2, IRb_exon3)   [with /trans_splicing]
          2. join(LSC_exon1, IRa_exon2, IRa_exon3)   [with /trans_splicing]

        The direction (strand) of each fragment is preserved exactly as PGA
        determined it — only the joining is changed.
        """
        try:
            from Bio import SeqIO
            ...
            # ------------------------------------------------------------------
            # 1. Collect all rps12 gene and CDS features
            # ------------------------------------------------------------------
            rps12_gene_feats = []
            rps12_cds_feats  = []
            other_feats      = []
            ...
            for feat in record.features:
                gene_qual = feat.qualifiers.get("gene", [""])[0]
                if gene_qual == "rps12":
                    if feat.type == "gene":
                        rps12_gene_feats.append(feat)
                    elif feat.type == "CDS":
                        rps12_cds_feats.append(feat)
                    else:
                        other_feats.append(feat)
                else:
                    other_feats.append(feat)

            if not rps12_gene_feats:
                self.logger.debug(f"No rps12 features found in {genbank_file.name}, skipping rps12 fix.")
                return

            # ------------------------------------------------------------------
            # 2. Identify LSC, IRb, IRA boundaries from misc_feature annotations
            #    added by _add_structural_features
            # ------------------------------------------------------------------
            lsc_start = lsc_end = None
            irb_start = irb_end = None
            ira_start = ira_end = None

            for feat in record.features:
                if feat.type == "misc_feature":
                    note = feat.qualifiers.get("note", [""])[0].lower()
                    s = int(feat.location.start)   # 0-based
                    e = int(feat.location.end)      # 0-based end (exclusive)
                    if "large single copy" in note or "lsc" in note:
                        lsc_start, lsc_end = s, e
                    elif "small single copy" in note or "ssc" in note:
                        pass  # SSC not needed for rps12
                elif feat.type == "repeat_region":
                    note = feat.qualifiers.get("note", [""])[0].lower()
                    s = int(feat.location.start)
                    e = int(feat.location.end)
                    if "irb" in note or "inverted repeat b" in note:
                        irb_start, irb_end = s, e
                    elif "ira" in note or "inverted repeat a" in note:
                        ira_start, ira_end = s, e

            if None in (lsc_start, lsc_end, irb_start, irb_end, ira_start, ira_end):
                self.logger.warning(
                    f"Could not determine LSC/IRb/IRa boundaries in {genbank_file.name}. "
                    "rps12 fix skipped. Make sure structural features are annotated first."
                )
                return

            # ------------------------------------------------------------------
            # 3. Categorise each rps12 CDS/gene fragment into LSC, IRb, or IRa
            #    A feature belongs to a region if its midpoint falls within that region.
            # ------------------------------------------------------------------
            def midpoint(feat):
                return (int(feat.location.start) + int(feat.location.end)) / 2

            def region_of(feat):
                mp = midpoint(feat)
                if lsc_start <= mp < lsc_end:
                    return "LSC"
                elif irb_start <= mp < irb_end:
                    return "IRB"
                elif ira_start <= mp < ira_end:
                    return "IRA"
                else:
                    return "OTHER"

            # Separate gene-level features
            lsc_gene_feats = [f for f in rps12_gene_feats if region_of(f) == "LSC"]
            irb_gene_feats = sorted([f for f in rps12_gene_feats if region_of(f) == "IRB"],
                                    key=lambda f: int(f.location.start))
            ira_gene_feats = sorted([f for f in rps12_gene_feats if region_of(f) == "IRA"],
                                    key=lambda f: int(f.location.start))

            # Separate CDS-level features
            lsc_cds_feats = [f for f in rps12_cds_feats if region_of(f) == "LSC"]
            irb_cds_feats = sorted([f for f in rps12_cds_feats if region_of(f) == "IRB"],
                                   key=lambda f: int(f.location.start))
            ira_cds_feats = sorted([f for f in rps12_cds_feats if region_of(f) == "IRA"],
                                   key=lambda f: int(f.location.start))

            if not lsc_gene_feats:
                self.logger.warning(
                    f"No rps12 LSC (exon 1) feature found in {genbank_file.name}. "
                    "rps12 fix skipped."
                )
                return

            if not irb_gene_feats and not ira_gene_feats:
                self.logger.warning(
                    f"No rps12 IR features found in {genbank_file.name}. "
                    "rps12 fix skipped."
                )
                return

            # Use the first LSC fragment (exon 1)
            lsc_gene = lsc_gene_feats[0]
            lsc_cds  = lsc_cds_feats[0] if lsc_cds_feats else lsc_gene

            # ------------------------------------------------------------------
            # 4. Helper: extract all individual FeatureLocation parts from a feature
            #    (handles simple and compound/join locations)
            # ------------------------------------------------------------------
            def get_parts(feat):
                loc = feat.location
            ...
            lsc_gene_parts = get_parts(lsc_gene)
            lsc_cds_parts  = get_parts(lsc_cds)

            # ------------------------------------------------------------------
            # 5. Build qualifier templates from existing features
            # ------------------------------------------------------------------
            def base_qualifiers(feat):
                """Return qualifiers dict without codon_start/transl_table/translation."""
                q = {k: list(v) for k, v in feat.qualifiers.items()
                     if k not in ("codon_start", "transl_table", "translation", "protein_id")}
                q["gene"] = ["rps12"]
                q["trans_splicing"] = [""]
                return q

            def cds_qualifiers(feat):
                q = base_qualifiers(feat)
                for key in ("codon_start", "transl_table", "product"):
                    if key in feat.qualifiers:
                        q[key] = list(feat.qualifiers[key])
                    else:
                        if key == "codon_start":
                            q[key] = ["1"]
                        elif key == "transl_table":
                            q[key] = ["11"]
                        elif key == "product":
                            q[key] = ["ribosomal protein S12"]
                return q

            # ------------------------------------------------------------------
            # 6. Create two pairs of joined gene+CDS features
            # ------------------------------------------------------------------
            new_features = []

            def make_joined_gene(lsc_parts, ir_feats):
                ir_parts = []
                for f in ir_feats:
                    ir_parts.extend(get_parts(f))
                all_parts = lsc_parts + ir_parts
                if len(all_parts) == 1:
                    loc = all_parts[0]
                else:
                    loc = CompoundLocation(all_parts)
                q = base_qualifiers(lsc_gene)
                return SeqFeature(loc, type="gene", qualifiers=q)

            def make_joined_cds(lsc_parts, ir_feats):
                ir_parts = []
                for f in ir_feats:
                    ir_parts.extend(get_parts(f))
                all_parts = lsc_parts + ir_parts
                if len(all_parts) == 1:
                    ...
                    loc = CompoundLocation(all_parts)
                q = cds_qualifiers(lsc_cds)
                return SeqFeature(loc, type="CDS", qualifiers=q)

            # IRb copy  (LSC exon1 + IRb exons)
            if irb_gene_feats:
                irb_gene_feat = make_joined_gene(lsc_gene_parts, irb_gene_feats)
                irb_cds_feat  = make_joined_cds(lsc_cds_parts,  irb_cds_feats if irb_cds_feats else irb_gene_feats)
                new_features.append(irb_gene_feat)
                new_features.append(irb_cds_feat)

            # IRa copy  (LSC exon1 + IRa exons)
            if ira_gene_feats:
                ira_gene_feat = make_joined_gene(lsc_gene_parts, ira_gene_feats)
                ira_cds_feat  = make_joined_cds(lsc_cds_parts,  ira_cds_feats if ira_cds_feats else ira_gene_feats)
                new_features.append(ira_gene_feat)
                new_features.append(ira_cds_feat)

            # ------------------------------------------------------------------
            # 7. Rebuild record.features: keep all non-rps12 features, append new ones
            # ------------------------------------------------------------------
            record.features = other_feats + new_features

            # Sort features by start position (standard GenBank ordering)
            record.features.sort(key=lambda f: (int(f.location.start), f.type))

            SeqIO.write(record, genbank_file, "genbank")
            self.logger.info(
                f"✓ Fixed rps12 trans-splicing in {genbank_file.name} "
                f"(IRb copy: {'yes' if irb_gene_feats else 'no'}, "
                f"IRa copy: {'yes' if ira_gene_feats else 'no'})"
            )

        except ImportError:
            self.logger.warning("Biopython not installed. Skipping rps12 trans-splicing fix.")
        except Exception as e:
            self.logger.warning(f"Failed to fix rps12 trans-splicing in {genbank_file.name}: {e}")
            import traceback
            self.logger.debug(traceback.format_exc())
                        ...
                        if pga_output_dir.exists():
                            ...
                            for f in output_files:
                                if f.suffix.lower() in ['.gb', '.gbk']:
                                    ...
                                    self._fix_rps12_trans_splicing(genbank_file)
